...
export default function SchoolManagement() {
  ...
  const isSchoolManager = teacher?.account_role === 'school_manager';
  const isManager = isSchoolManager && selectedMembership?.role === 'school_admin';
  ...
  if (!schools.length) return <div className="school-shell" dir={direction}><div className="school-shell__topbar">{isSchoolManager ? <button type="button" className="school-shell__back school-shell__logout" onClick={logout}><Icon name="logout" className="w-4 h-4" />{t('logout')}</button> : <Link to="/" className="school-shell__back"><Icon name="arrowLeft" className="w-4 h-4" />{t('schoolReturnToSpace')}</Link>}<span className="school-shell__brand"><Icon name="school" className="w-5 h-5" />EduCore</span></div>{renderNoSchool()}</div>;

  return <div className="school-shell" dir={direction}>
    <header className="school-shell__topbar"><div className="school-shell__topbar-main">{isSchoolManager ? <button type="button" className="school-shell__back school-shell__logout" onClick={logout}><Icon name="logout" className="w-4 h-4" />{t('logout')}</button> : <Link to="/" className="school-shell__back"><Icon name="arrowLeft" className="w-4 h-4" />{t('schoolReturnToSpace')}</Link>}<span className="school-shell__brand"><Icon name="school" className="w-5 h-5" />EduCore</span></div><div className="school-shell__account"><span className="school-shell__account-avatar">{String(teacher?.full_name || teacher?.email || '?').trim().charAt(0)}</span><span>{isManager ? t('schoolRoleAdmin') : teacher?.subject || t('schoolRoleTeacher')}</span><small className="school-shell__sync-meta">{lastSyncAt ? displayDateTime(lastSyncAt, locale) : t('syncBackground')}</small><button type="button" className="school-shell__sync-button" onClick={() => void syncNow()} disabled={syncing} title={syncing ? t('syncing') : t('syncNow')}><Icon name="refresh" className={`w-4 h-4 ${syncing ? 'animate-spin' : ''}`} /><span>{syncing ? t('syncing') : t('syncNow')}</span></button></div></header>
