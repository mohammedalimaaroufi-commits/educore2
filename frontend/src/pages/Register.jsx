...
export default function Register() {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || t('requiredFields'));
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="register-shell">
      <div className="register-aside">
        <span className="register-brand">EduCore Manager</span>
      ...
      <div className="register-card">
        <div className="register-card__top"><div><span className="settings-eyebrow">EduCore Manager</span><h2>{t('registerTitle')}</h2></div><label className="register-language"><span>{t('languageChoice')}</span><select className="input" value={form.locale} onChange={updateLocale}><option value="ar">العربية</option><option value="en">English</option></select></label></div>
        ...
        <form onSubmit={submit} className="register-form">
          ...
          <div className="register-form-grid"><label className="label">{t('subject')}<input className="input" value={form.subject} onChange={update('subject')} placeholder={locale === 'ar' ? 'مثال: الرياضيات' : 'e.g. Mathematics'} /></label><label className="label">{t('schoolStage')}<input className="input" value={form.school_stage} onChange={update('school_stage')} placeholder={locale === 'ar' ? 'مثال: الصف الخامس' : 'e.g. Grade 5'} /></label></div>
