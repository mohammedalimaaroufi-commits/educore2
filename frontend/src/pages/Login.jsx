    ...
    <div className="auth-shell" dir={ar ? 'rtl' : 'ltr'}>
      ...
      <div className="auth-layout">
        <section className="auth-intro">
          ...
          <div className="auth-platforms"><div className="auth-platforms__heading"><strong>{ar ? 'منصات EduCore' : 'EduCore platforms'}</strong><small>{ar ? 'النسخة الأحدث من Web متاحة الآن، وتطبيقات الهاتف في الطريق.' : 'The latest Web version is available now; mobile apps are on the way.'}</small></div><div className="auth-platforms__grid"><div className="auth-platform-card auth-platform-card--large is-live"><a href={window.location.origin} aria-label={ar ? 'فتح نسخة الويب' : 'Open web version'}><span className="auth-platform-card__icon">◎</span><strong>Web / PWA</strong><small>{ar ? 'متاح الآن · فتح المنصة' : 'Available now · Open platform'}</small><span className="auth-platform-card__action">↗</span></a><PwaInstallButton compact /></div><div className="auth-platform-card auth-platform-card--large is-soon" aria-label={ar ? 'تطبيق Android قريبًا' : 'Android app coming soon'}><span className="auth-platform-card__icon">A</span><strong>Android</strong><small>{ar ? 'قريبًا' : 'Coming soon'}</small><span className="auth-platform-card__action">…</span></div><div className="auth-platform-card auth-platform-card--large is-future" aria-label={ar ? 'تطبيق iOS خطة مستقبلية' : 'iOS future plan'}><span className="auth-platform-card__icon"></span><strong>iOS</strong><small>{ar ? 'خطة مستقبلية' : 'Future plan'}</small><span className="auth-platform-card__action">·</span></div></div></div>
        </section>
        <section className="auth-panel">
          ...
          <form onSubmit={submit} className="auth-form">
            <div><label className="label">{t('email')}</label><input className="input" type="email" value={email} onChange={(e) => setEmail(e.target.value)} required autoComplete="email" /></div>
            <div><label className="label">{t('password')}</label><input className="input" type="password" value={password} onChange={(e) => setPassword(e.target.value)} required autoComplete="current-password" /></div>
          ...
          <div className="auth-socials"><button className="btn-secondary" type="button" disabled>Google <span>{ar ? 'قريبًا' : 'Soon'}</span></button><button className="btn-secondary" type="button" disabled>Apple ID <span>{ar ? 'قريبًا' : 'Soon'}</span></button></div>
