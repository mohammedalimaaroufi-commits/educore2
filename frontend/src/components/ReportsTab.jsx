...
function ClassReport({ snapshot, classId, className }) {
  ...
  return (
    <div>
      <div className="reports-section-head print:hidden">
        <h3 className="reports-section-title"><Icon name="reports" className="w-5 h-5" />{t('reportsClassTitle')}</h3>
        <div className="reports-section-actions"><button className="btn-secondary text-sm" onClick={exportCSV}><Icon name="reports" className="w-4 h-4" />{t('reportsExportCsv')}</button><button className="btn-primary text-sm" onClick={printClassReport}><Icon name="fileCheck" className="w-4 h-4" />{t('reportsExportPdf')}</button></div>
      </div>
      ...
      <div className="report-filters reports-filter-bar print:hidden"><div className="analytics-search"><span>⌕</span><input value={studentSearch} onChange={(event) => setStudentSearch(event.target.value)} placeholder={t('reportsSearchPlaceholder')} /></div><select className="input text-sm" value={reportFilter} onChange={(event) => setReportFilter(event.target.value)}><option value="all">{t('reportsAllStudents')}</option><option value="graded">{t('reportsGraded')}</option><option value="missing">{t('reportsMissingGrade')}</option><option value="low-attendance">{t('reportsLowAttendance')}</option><option value="negative-behavior">{t('reportsNegativeBehavior')}</option><option value="follow-up">{t('reportsNeedsFollowUp')}</option></select><span className="reports-filter-count">{t('reportsDisplaying', '', { visible: visibleRoster.length, total: roster.length })}</span></div>
      <section className="reports-class-summary card p-6 mb-6">
        <div className="reports-summary-heading"><div><span className="reports-eyebrow">{t('reportsClassTitle')}</span><h2 className="text-xl font-bold mb-1">{classData?.name || className}</h2></div><span className="reports-local-badge">{t('reportsCalculatedLocally')}</span></div>
                <p className="text-ink/60 text-sm mb-4">{t('reportsDate')}: {new Date().toLocaleDateString(isEnglish ? 'en-US' : 'ar')}</p>
        <div className="reports-table-scroll table-scroll-sticky max-h-80"><table className="table-head-sticky w-full text-sm">
<thead><tr><th className="text-right px-4 py-2">{t('reportsStudent')}</th><th className="text-right px-4 py-2">{t('reportsGrade')}</th><th className="text-right px-4 py-2">{t('reportsBehaviorPoints')}</th><th className="text-right px-4 py-2">{t('reportsAttendanceRate')}</th></tr></thead><tbody>{visibleRoster.map((row) => <tr key={row.student_id} className="border-t border-line"><td className="px-4 py-2">{row.full_name}</td><td className="px-4 py-2 font-bold text-primary">{row.finalGrade !== null ? `${row.finalGrade}%` : '—'}</td><td className={`px-4 py-2 ${row.behaviorScore >= 0 ? 'text-primary' : 'text-danger'}`}>{row.behaviorScore}</td><td className="px-4 py-2">{row.attendanceRate !== null ? `${row.attendanceRate}%` : '—'}</td></tr>)}</tbody></table></div>
      ...
      {gradeChart.length > 0 && <section className="reports-grade-chart-card card p-4"><h3 className="reports-card-title"><Icon name="analytics" className="w-4 h-4" />{t('reportsCompareGrades')}</h3><div className="reports-chart-scroll"><ResponsiveContainer width="100%" height={Math.max(220, Math.min(gradeChart.length * 36, 760))}><BarChart data={gradeChart} layout="vertical" margin={{ top: 12, right: 48, left: 20, bottom: 10 }}><CartesianGrid strokeDasharray="3 3" stroke="#E4E1D8" /><XAxis type="number" domain={[0, 100]} tick={{ fontSize: 11 }} /><YAxis type="category" dataKey="name" width={150} tick={{ fontSize: 11 }} /><Tooltip /><Bar dataKey="grade" fill="#2E7D6B" radius={[0, 6, 6, 0]} name={`${t('reportsGrade')} %`}><LabelList dataKey="grade" position="right" formatter={(value) => `${value}%`} fill="#1B2430" fontSize={11} fontWeight={700} /></Bar></BarChart></ResponsiveContainer></div></section>}
    </div>
  );
}
...
function StudentReport({ snapshot, studentId }) {
  ...
  return <div className="reports-student-workspace">
    <div className="reports-section-head print:hidden"><h3 className="reports-section-title"><Icon name="reports" className="w-5 h-5" />{t('reportsUnifiedStudent')}</h3><button className="btn-primary text-sm" onClick={printStudentReport}><Icon name="fileCheck" className="w-4 h-4" />{t('reportsExportPdf')}</button></div>
    <section className="reports-student-summary card p-6 mb-6"><div className="reports-summary-heading"><div><span className="reports-eyebrow">{t('reportsUnifiedStudent')}</span><h2 className="text-xl font-bold mb-1">{report.student.full_name}</h2><p className="text-ink/60 text-sm">{report.class?.name} — {t('reportsLocalReport')}</p></div><span className="reports-local-badge">{t('reportsCalculatedLocally')}</span></div><div className="reports-student-kpis"><div><p>{t('reportsGrade')}</p><strong className="text-primary">{report.finalGrade !== null ? `${report.finalGrade}%` : '—'}</strong></div><div><p>{t('reportsBehaviorPoints')}</p><strong className="text-primary">{report.behaviorScore}</strong></div><div><p>{t('reportsAttendanceRecords')}</p><strong>{report.attendance.length}</strong></div><div><p>{t('reportsCategories')}</p><strong>{report.gradesByCategory.length}</strong></div></div>{report.autoRecommendation && <div className="reports-recommendation"><span>{t('reportsTeacherRecommendation')}</span>{report.autoRecommendation}</div>}</section>
    <div className="reports-student-charts"><section className="reports-chart-card card p-4"><h4 className="reports-card-title"><Icon name="analytics" className="w-4 h-4" />{t('reportsAverageByCategory')}</h4><ResponsiveContainer width="100%" height={250}><BarChart data={categoryBars} margin={{ top: 22, right: 18, left: 8, bottom: 44 }}><CartesianGrid strokeDasharray="3 3" stroke="#E4E1D8" /><XAxis dataKey="name" interval={0} angle={-24} textAnchor="end" height={56} tick={{ fontSize: 10 }} /><YAxis domain={[0, 100]} tick={{ fontSize: 10 }} /><Tooltip /><Bar dataKey="percent" fill="#2E7D6B" radius={[6, 6, 0, 0]} name={`${t('reportsAverageGrade')} %`}><LabelList dataKey="percent" position="top" formatter={(value) => `${value}%`} fill="#1B2430" fontSize={10} fontWeight={700} /></Bar></BarChart></ResponsiveContainer></section>{growth.length > 0 && <section className="reports-chart-card card p-4"><h4 className="reports-card-title"><Icon name="analytics" className="w-4 h-4" />{t('reportsAcademicGrowth')}</h4><ResponsiveContainer width="100%" height={240}><LineChart data={growth} margin={{ top: 24, right: 18, left: 8, bottom: 18 }}><CartesianGrid strokeDasharray="3 3" stroke="#E4E1D8" /><XAxis dataKey="index" tick={{ fontSize: 10 }} /><YAxis domain={[0, 100]} tick={{ fontSize: 10 }} /><Tooltip /><Line type="monotone" dataKey="percent" stroke="#E0A548" strokeWidth={3} dot={{ r: 3 }}><LabelList dataKey="percent" position="top" formatter={(value) => `${value}%`} fill="#1B2430" fontSize={10} fontWeight={700} /></Line></LineChart></ResponsiveContainer></section>}{behaviorPie.length > 0 && <section className="reports-chart-card card p-4"><h4 className="reports-card-title"><Icon name="analytics" className="w-4 h-4" />{t('reportsBehaviorDistribution')}</h4><ResponsiveContainer width="100%" height={200}><PieChart><Pie data={behaviorPie} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={62} label={renderPieLabel} labelLine={{ stroke: '#8DAAA1', strokeWidth: 1 }}>{behaviorPie.map((_, index) => <Cell key={index} fill={PIE_COLORS[index % PIE_COLORS.length]} />)}</Pie><Tooltip /><Legend /></PieChart></ResponsiveContainer></section>}{attendancePie.length > 0 && <section className="reports-chart-card card p-4"><h4 className="reports-card-title"><Icon name="analytics" className="w-4 h-4" />{t('reportsAttendanceDistribution')}</h4><ResponsiveContainer width="100%" height={200}><PieChart><Pie data={attendancePie} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={62} label={renderPieLabel} labelLine={{ stroke: '#8DAAA1', strokeWidth: 1 }}>{attendancePie.map((_, index) => <Cell key={index} fill={PIE_COLORS[index % PIE_COLORS.length]} />)}</Pie><Tooltip /><Legend /></PieChart></ResponsiveContainer></section>}</div>
  </div>;
}
