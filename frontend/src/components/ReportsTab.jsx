...
function StudentReport({ snapshot, studentId }) {
  ...
  return <div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6"><div className="card p-4"><h4 className="font-bold text-sm mb-1">متوسط الدرجات حسب الفئة</h4><ReportChartKey label="الفئات" items={categoryKey} /><ResponsiveContainer width="100%" height={250}><BarChart data={categoryBars} margin={{ top: 22, right: 18, left: 8, bottom: 44 }}><CartesianGrid strokeDasharray="3 3" stroke="#E4E1D8" /><XAxis dataKey="name" interval={0} angle={-24} textAnchor="end" height={56} tick={{ fontSize: 10 }} /><YAxis domain={[0, 100]} tick={{ fontSize: 10 }} /><Tooltip /><Bar dataKey="percent" fill="#2E7D6B" radius={[6, 6, 0, 0]} name="المتوسط %"><LabelList dataKey="percent" position="top" formatter={(value) => `${value}%`} fill="#1B2430" fontSize={10} fontWeight={700} /></Bar></BarChart></ResponsiveContainer></div>{growth.length > 0 && <div className="card p-4"><h4 className="font-bold text-sm mb-1">النمو الأكاديمي عبر الزمن</h4><ReportChartKey label="الفئات الظاهرة" items={growthKey} /><ResponsiveContainer width="100%" height={240}><LineChart data={growth} margin={{ top: 24, right: 18, left: 8, bottom: 18 }}><CartesianGrid strokeDasharray="3 3" stroke="#E4E1D8" /><XAxis dataKey="index" tick={{ fontSize: 10 }} /><YAxis domain={[0, 100]} tick={{ fontSize: 10 }} /><Tooltip /><Line type="monotone" dataKey="percent" stroke="#E0A548" strokeWidth={3} dot={{ r: 3 }}><LabelList dataKey="percent" position="top" formatter={(value) => `${value}%`} fill="#1B2430" fontSize={10} fontWeight={700} /></Line></LineChart></ResponsiveContainer></div>}{behaviorPie.length > 0 && <div className="card p-4"><h4 className="font-bold text-sm mb-1">توزيع السلوك</h4><ReportChartKey label="الفئات" items={behaviorPie.map((item) => ({ id: item.name, label: item.name, meta: `${item.value} سجل` }))} /><ResponsiveContainer width="100%" height={200}><PieChart><Pie data={behaviorPie} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={62} label={renderPieLabel} labelLine={{ stroke: '#8DAAA1', strokeWidth: 1 }}>{behaviorPie.map((_, index) => <Cell key={index} fill={PIE_COLORS[index % PIE_COLORS.length]} />)}</Pie><Tooltip /><Legend /></PieChart></ResponsiveContainer></div>}{attendancePie.length > 0 && <div className="card p-4"><h4 className="font-bold text-sm mb-1">توزيع الحضور</h4><ReportChartKey label="الحالات" items={attendancePie.map((item) => ({ id: item.name, label: item.name, meta: `${item.value} سجل` }))} /><ResponsiveContainer width="100%" height={200}><PieChart><Pie data={attendancePie} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={62} label={renderPieLabel} labelLine={{ stroke: '#8DAAA1', strokeWidth: 1 }}>{attendancePie.map((_, index) => <Cell key={index} fill={PIE_COLORS[index % PIE_COLORS.length]} />)}</Pie><Tooltip /><Legend /></PieChart></ResponsiveContainer></div>}</div>
  </div>;
}
