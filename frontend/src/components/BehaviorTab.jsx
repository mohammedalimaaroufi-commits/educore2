...
function buildSummary(snapshot, classId) {
  ...
  return students.map((student) => {
    const logs = (snapshot?.behavior_logs || []).filter((log) => log.student_id === student.id);
    return {
...
export default function BehaviorTab({ classId }) {
  ...
  return <div className="space-y-6">
    <div className="card p-4"><div className="flex items-center justify-between mb-3 flex-wrap gap-2"><h3 className="font-bold">رصد سلوك بنقرة واحدة</h3>{feedback && <span className="text-primary text-sm">{feedback}</span>}</div><div className="relative mb-3"><input className="input text-sm pr-9" placeholder="بحث سريع عن طالب..." value={query} onChange={(event) => setQuery(event.target.value)} /><Icon name="search" className="w-4 h-4 absolute right-3 top-1/2 -translate-y-1/2 text-ink/30" /></div><div className="space-y-2">{filteredStudents.map((student) => <div key={student.id} className="border border-line rounded-xl2 overflow-hidden"><button onClick={() => setOpenStudent((current) => (current === student.id ? null : student.id))} className={`w-full flex items-center gap-2 px-3 py-2.5 text-sm text-right hover:bg-surface ${openStudent === student.id ? 'bg-surface' : ''}`}><StudentAvatar name={student.full_name} photoUrl={student.photo_url} size={26} /><span className="flex-1">{student.full_name}</span><Icon name={openStudent === student.id ? 'chevronUp' : 'chevronDown'} className="w-4 h-4 text-ink/40" /></button>{openStudent === student.id && <div className="px-3 pb-3 pt-1 border-t border-line bg-surface/50"><p className="text-xs text-ink/50 mb-2">اضغط على أي ملاحظة سلوكية محفوظة لرصدها فورًا لـ{student.full_name}:</p><div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mb-3">{types.map((type) => <button key={type.id} onClick={() => logBehavior(student.id, type.id)} className={`px-3 py-2 rounded-xl2 text-sm font-medium border flex items-center justify-center gap-2 ${type.polarity === 'positive' ? 'border-primary/40 text-primary hover:bg-primary/10' : 'border-danger/40 text-danger hover:bg-danger/10'}`}><Icon name={type.icon} className="w-4 h-4" />{type.label} ({type.points > 0 ? '+' : ''}{type.points})</button>)}</div><input className="input text-xs" placeholder="ملاحظة سريعة (اختياري)" value={noteDrafts[student.id] || ''} onChange={(event) => setNoteDrafts((drafts) => ({ ...drafts, [student.id]: event.target.value }))} /><button className="text-primary text-xs mt-2" onClick={() => setDetailStudentId(student.id)}>عرض كامل السجل السلوكي لهذا الطالب</button></div>}</div>)}{filteredStudents.length === 0 && <p className="text-ink/50 text-sm py-4 text-center">لا يوجد طالب مطابق للبحث.</p>}</div>{!showTypeForm ? <button className="text-primary text-sm mt-3" onClick={() => setShowTypeForm(true)}>+ إضافة سلوك مخصص لهذا الصف</button> : <form onSubmit={addType} className="space-y-2 mt-3 pt-3 border-t border-line"><div className="flex flex-wrap gap-2"><input className="input text-sm flex-1" placeholder="اسم السلوك" required value={newType.label} onChange={(event) => setNewType({ ...newType, label: event.target.value })} /><select className="input text-sm w-32" value={newType.polarity} onChange={(event) => setNewType({ ...newType, polarity: event.target.value, icon: event.target.value === 'positive' ? 'star' : 'clock' })}><option value="positive">إيجابي</option><option value="negative">سلبي</option></select><input className="input text-sm w-20" type="number" value={newType.points} onChange={(event) => setNewType({ ...newType, points: Number(event.target.value) })} /></div><div className="flex gap-2">{iconOptions.map((icon) => <button key={icon} type="button" onClick={() => setNewType({ ...newType, icon })} className={`p-2 rounded-lg border ${newType.icon === icon ? 'border-primary bg-primary/10' : 'border-line'}`}><Icon name={icon} className="w-4 h-4" /></button>)}<button className="btn-secondary text-sm mr-auto" type="submit">حفظ السلوك</button></div></form>}</div>
    <div className="card p-4"><h3 className="font-bold mb-3">مؤشر السلوك للفصل</h3><div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-surface"><tr><th className="text-right px-4 py-2">الطالب</th><th className="text-right px-4 py-2">النقاط</th><th className="text-right px-4 py-2">إيجابي</th><th className="text-right px-4 py-2">سلبي</th><th className="px-4 py-2"></th></tr></thead><tbody>{summary.map((row) => <tr key={row.student_id} className="border-t border-line"><td className="px-4 py-2">{row.full_name}</td><td className={`px-4 py-2 font-bold ${row.behavior_score >= 0 ? 'text-primary' : 'text-danger'}`}>{row.behavior_score}</td><td className="px-4 py-2 text-primary flex items-center gap-1"><Icon name="thumbsUp" className="w-3.5 h-3.5" />{row.positive_count}</td><td className="px-4 py-2 text-danger flex items-center gap-1"><Icon name="thumbsDown" className="w-3.5 h-3.5" />{row.negative_count}</td><td className="px-4 py-2 text-left"><button className="text-primary text-xs" onClick={() => setDetailStudentId(row.student_id)}>التفاصيل الكاملة</button></td></tr>)}</tbody></table></div></div>
