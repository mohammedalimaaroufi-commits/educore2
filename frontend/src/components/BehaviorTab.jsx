...
export default function BehaviorTab({ classId, schoolClass = false }) {
      ...
      <div className="card behavior-control-card">
        <div className="behavior-command-bar">
          ...
          <div className="behavior-command-controls">
            {!schoolClass && <button type="button" className="behavior-custom-trigger" onClick={() => setShowTypeForm(true)}><Icon name="plus" className="w-4 h-4" /><span>{t('addCustomBehavior')}</span></button>}
        ...
        <p className="behavior-results-summary">{t('displayCount', '', { visible: filteredStudents.length, total: students.length })} · {t('behaviorScoreExplanation')}</p>
        ...
        {!schoolClass && editingTypeId && editingType && <form className="behavior-type-editor mt-3 pt-3 border-t border-line space-y-2" onSubmit={(event) => { event.preventDefault(); void updateType(editingTypeId); }}>
